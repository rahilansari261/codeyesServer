...
class OurTeamController {
  ...
  static AddOurTeam = async (req, resp) => {
    try {
      ...
      if (image && image.image_url) {
        newOurTeam.image_url = `${base_url}/${image.image_url[0].path.replace(/\\/g, "/")}`;
      }
  ...
  static UpdateOurTeam = async (req, resp) => {
    try {
      const { id } = req.params;
      const { name, position, twitter_link, facebook_link, linkedin_link, instagram_link, title } = req.body;
      const image = req.files;
