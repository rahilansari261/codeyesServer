...
import HomeLogo from "../src/models/HomeLogo.js";
class HomeLogoController {
  // Create news
  ...
  static getAllLogos = async (req, resp) => {
    try {
      const news = await HomeLogo.find({ deleted_at: null, organizationId: req.query.organizationId });
  ...
  static getLogoById = async (req, resp) => {
    try {
      const { id } = req.params;
      const news = await HomeLogo.findOne({ id: id });
      if (!news) {
...
export default HomeLogoController;
