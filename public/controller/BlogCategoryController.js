...
class BlogCategoryController {
  ...
  static GetAllBlogCategoriesController = async (req, resp) => {
    try {
      const blogCategories = await BlogCategory.find({ deleted_at: null, organizationId: req.query.organizationId }).lean();
  ...
  static UpdateBlogCategoryController = async (req, resp) => {
    try {
      ...
      const updatedBlogCategory = await BlogCategory.findOne({ id: id, deleted_at: null });
  ...
  static DeleteBlogCategoryController = async (req, resp) => {
    try {
      ...
      const deletedBlogCategory = await BlogCategory.findOneAndUpdate({ id: id }, { deleted_at: new Date() }, { new: true });
